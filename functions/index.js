...
exports.onCreateFollower = functions.firestore.document("/followers/{userId}/userFollowers/{followerId}")
    .onCreate(async (snapshot, context) => {
        ...
        const timelinePostsRef = admin.firestore().
            collection('timeline').doc(followerId).
...
exports.onUpdatePost = functions.firestore
    ...
    onUpdate(async (change, context) => {
        ...
        querySnapshot.forEach(doc => {
            ...
            admin.firestore().collection('timeline').doc('followerId')
                .collection('timelinePosts').doc(postId).get.then(
                    (doc) => {
                        if (doc.exists) {
...
.onCreate( async (snapshot, context) => {
    ...
    const usedId = context.params.userId;
    ...
    if (androidNotificationToken) {
        //send noti
        sendNotifications(androidNotificationToken, createdActivityFeedItem)
    } else {
        console.log("no token for user, cannot send notifications")
    }

    function sendNotifications(androidNotificationToken, activityFeedItem) {
        let body;

        //switch body value based on the notification type
        ...
        admin.messaging().send(meesage).then( (response) => {
